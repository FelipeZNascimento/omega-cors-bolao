...
async function checkExistingValues(email, name, logedUserId = "") {
  const allQueries = [
...
exports.updatePreferences = async function (req, res) {
  ...
  try {
    ...
    await User.setIcons(user.id, userData.icon, userData.color).then(
      async () => {
        await User.getById(user.id).then((user) => {
...
exports.update = async function (req, res) {
  ...
  try {
    ...
    const allQueries = [User.updateInfo(user.id, userData)];
    ...
    await User.getById(user.id).then((user) => {
      ...
      const returnObject = {
        ...
        changedPassword:
          allResults.length > 1 ? allResults[1].value.affectedRows : false,
      };
...
exports.listById = async function (req, res) {
  ...
  try {
    await User.getById(id).then((user) => {
      res.send(user);
    });
  } catch (err) {
    res.status(400).send(err.message);
...
exports.register = async function register(req, res) {
  ...
  try {
    ...
    await User.register(userData).then(async (result) => {
      const allQueries = [
      ...
      await User.login(userData).then((loginResult) => {
