...
const TeamController = require("./team.js");
const cachedInfo = require("../utilities/cache.js");
const CACHE_KEYS = require("../const/cacheValues.js");
const SEASON_MAPPING = require("../const/seasonMapping.js");
...
exports.listExtraBets = async function (req, res) {
  ...
  const allQueries = [Bets.userExtraBets(normalizedSeason, user.id)];
  ...
  const userExtraBets = allResults[0].value[0]
    ? JSON.parse(allResults[0].value[0].json)
    : null;
  ...
  if (nowTimestamp >= seasonStart) {
    extraBetsResults =
      allResults[1].value.length > 0
        ? JSON.parse(allResults[1].value[0].json)
        : null;
            ...
            if (err) {
              ...
              const matchesObject = matches.map((match) => {
                ...
                const awayTeam = teams.find(
                  (team) => team.id === match.idTeamAway
                );
                const homeTeam = teams.find(
                  (team) => team.id === match.idTeamHome
                );
                return {
                  id: match.id,
...
exports.updateExtraBets = async function (req, res) {
  ...
  try {
    ...
    const { user } = req.session;
    await Bets.updateExtras(
      user.id,
      normalizedSeason,
      JSON.stringify(newExtraBets)
    ).then(async (result) => {
      User.updateLastOnlineTime(user.id);
      res.send(result);
    });
...
exports.updateRegularBets = async function (req, res) {
  ...
  try {
    ...
    await Match.getById(betData.matchId).then(async (match) => {
      if (match.length > 0 && nowTimestamp >= match[0].timestamp) {
        throw new Error("Season already started, update extraBets not allowed");
      } else {
        await Bets.updateRegular(
          betData.matchId,
          user.id,
          betData.betValue
        ).then(async (result) => {
